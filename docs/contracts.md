...
| `runtime-platform-env` | The `env` command prints `BASE_OS`, `BASE_PLATFORM`, and `BASE_HOST` with the other Base runtime values. | `src/env.sh` | `tests/validate.sh`, `tests/demo_test.bats`, `demo/demo.sh` | Learners cannot discover the Base v1.5.0 platform contract from base-demo. | Runtime |
